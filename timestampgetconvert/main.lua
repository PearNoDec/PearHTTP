...
local function get_timestamp_13()
  local socket = require("socket")
  local t = socket.gettime()
  return math.floor(t * 1000)
end

function detect_and_convert_timestamp(input_timestamp)
  local len = string.len(input_timestamp)
  local time_num = tonumber(input_timestamp)
  ...
  if len == 10 then
    local time_str = os.date("%Y-%m-%d %H:%M:%S", time_num)
    return time_str

   elseif len == 13 then
    local seconds = math.floor(time_num / 1000)
    local milliseconds = time_num % 1000
    local time_str = os.date("%Y-%m-%d %H:%M:%S", seconds)
    return time_str

   else
    print("时间戳长度不是10位或13位")
  end
end

GetTen.onClick=function()
  local ResultEn = os.time()
  ResultText.Text = tostring(ResultEn)
end

GetThirteen.onClick=function()
  local ResultEn = get_timestamp_13()
  ResultText.Text = tostring(ResultEn)
end

GetConvert.onClick=function()
  local ChooseText = InputText.Text
  local ResultEn = detect_and_convert_timestamp(ChooseText)
  ResultText.Text = tostring(ResultEn)
end
